...
public class OpenAPIConfiguration {
  ...
  /**
   * Sorts tags (sections of the registry documentation) by the order extension, rather than
   * alphabetically.
   */
  @Bean
  public OpenApiCustomizer sortTagsByOrderExtension() {
    return openApi -> {
      ...
      openApi.setTags(openApi.getTags().stream().sorted(tagOrder()).collect(Collectors.toList()));
